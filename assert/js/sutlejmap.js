...
var sutlejgeojson;
function loadSutlej(sutlejUrl) {

    if (map.hasLayer(sutlejgeojson))
        map.removeLayer(sutlejgeojson)

    $.getJSON(sutlejUrl, function (data) {

        var objname = Object.keys(data.objects)[0];
        var sutlejTopoJson = topojson.feature(data, data.objects[objname])
        //sutlejTopoJson.features = sutlejTopoJson.features.map(function (fm, i) {
        //    var ret = fm;
        //    ret.ind = i;
        //    return ret
        //});

        sutlejgeojson = L.geoJson(sutlejTopoJson, {
            style: {
                color: "brown",
                fillColor: "#FFFFFF00", //color(feat.ind),
                fillOpacity: 0,
                interactive: false,
                weight: 2.5
            }
        ...
        //console.log(sutlejgeojson.getBounds())
        //map.fitBounds(sutlejgeojson.getBounds());

        //var locurl = 'json/sutlej/Project_Loc.json'
        //loadSutlejPointLayer(locurl)

    });
}
var bsnurl = 'json/sutlej/Basin.json'
loadSutlej(bsnurl);

//if (map.hasLayer(sutlejgeojson))
//    map.fitBounds(sutlejgeojson.getBounds());
var sutlejPointLayer;
function loadSutlejPointLayer(locurl) {

    if (map.hasLayer(sutlejPointLayer))
        map.removeLayer(sutlejPointLayer);

    ////*********************** Adding Historical ***********************
    if (fileExist(locurl) === 200) {

        $.getJSON(locurl, function (data) {

            var objname = Object.keys(data.objects)[0];
            var sutlejPointTopoJson = topojson.feature(data, data.objects[objname])

            sutlejPointLayer = L.geoJson(sutlejPointTopoJson, {
                onEachFeature: onEachSutlejPointLayer,
            ...
            //map.fitBounds(sutlejPointLayer.getBounds());

            function onEachSutlejPointLayer(feature, layer) {
                var msg = 'Dam Name : ' + feature.properties.Name;
                //console.log(msg);
...
//var rchurl = 'json/sutlej/Reach.json'
//loadSutlej(rchurl);
//var waturl = 'json/sutlej/Watershed.json'
//loadSutlej(waturl);
...
$.getJSON(indiaurl, function (data) {
    ...
    function loadBasin(e) {
        ...
        if (e.target.feature.properties.Code !== "IND") return;
        ...
        var mapSpin = setInterval(
            function () {
                if (map.hasLayer(subgeojson) && map.hasLayer(rchgeojson) && map.hasLayer(cwcgeojson)) {
                    ...
                    map.addLayer(sutlejgeojson);
                    map.addLayer(sutlejPointLayer);
    ...
    function onEachBasinLayer(feature, layer) {
        if (feature.properties.Code == "IND") {
...
function loaddata() {
    ...
    if (fileExist('./data/' + basincode + '_' + modelname + 'subwebsite.txt?t=' + new Date().getTime()) === 200) {
        readTextFile('./data/' + basincode + '_' + modelname + 'subwebsite.txt?t=' + new Date().getTime(), function (txt) {
            ...
            //console.log(daylst);
            //if (daylst.length > 2)
            //    document.getElementById('rundate').innerHTML = "Simulation used " + modelname.toUpperCase() + ' on ' + daylst[1];
